...
import { texts } from './translations'; // Importamos las traducciones

function Portfolio() {
  const [lang, setLang] = useState('es');
  const toggleLang = () => setLang(lang === 'es' ? 'en' : 'es');
  const t = texts[lang]; // Atajo para usar los textos
    ...
    <div className="min-h-screen bg-[#050505] text-white selection:bg-blue-600 font-sans relative">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-32">
        ...
        <header className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-40 animate-in fade-in slide-in-from-bottom-10 duration-1000">
          <div>
            ...
            <h1 className="text-7xl md:text-[110px] font-black leading-[0.8] tracking-tighter mb-10 uppercase">
              SEBASTIÁN<br/>SILVA<br/><span className="text-gray-500">{t.hero_subtitle}</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-5">
              <a href="#contacto" className="bg-white text-black px-10 py-4 rounded-full font-black hover:scale-105 transition-all uppercase text-sm">{t.btn_hire}</a>
              <a href="/Curriculum_final_20_03_2026.pdf" download className="px-10 py-4 border border-gray-800 rounded-full font-black hover:bg-white/5 transition-all uppercase text-sm text-center">{t.btn_cv}</a>
            </div>
          </div>
          ...
          <div className="relative aspect-[4/5] rounded-[2rem] overflow-hidden border border-white/5 group shadow-2xl">
             ...
             <div className="absolute bottom-10 left-10 z-20">
                <p className="text-[10px] font-mono text-blue-500 mb-2 tracking-[0.5em] uppercase">{t.location}</p>
             </div>
          </div>
        </header>
        ...
        <section id="trayectoria" className="mb-40">
          <h2 className="text-sm font-black text-gray-500 uppercase tracking-[0.4em] mb-16 px-4 border-l-2 border-blue-600">{t.trayectoria_title}</h2>
          <div className="space-y-1">
            {t.jobs.map((job, i) => (
              <div key={i} className="group grid grid-cols-1 md:grid-cols-[250px_1fr] gap-8 p-10 border-b border-white/5 hover:bg-white/[0.02] transition-all">
                <span className="text-gray-600 font-mono text-sm uppercase">{job.date}</span>
                <div>
                  <h3 className="text-2xl font-black uppercase group-hover:text-blue-500 transition-colors">{job.title}</h3>
                  <p className="text-gray-400 mt-2 max-w-2xl">{job.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {certifications.length > 0 && (
          <section className="mb-40 border-y border-white/5 py-12 overflow-hidden relative">
            <h2 className="text-[10px] font-black text-center text-gray-500 uppercase tracking-[0.4em] mb-10">{t.certs_title}</h2>
        ...
        <section id="proyectos" className="mb-40">
          <div className="flex justify-between items-end mb-20">
            <h2 className="text-6xl font-black tracking-tighter uppercase">{t.projects_title.split(' ')[0]}<br/>{t.projects_title.split(' ')[1]}<span className="text-blue-600">.</span></h2>
          </div>
          {projects.length === 0 ? (
            <div className="p-10 border border-white/5 rounded-[2rem] bg-white/[0.01] text-center">
              <p className="text-gray-500 font-mono tracking-widest text-sm uppercase animate-pulse">{t.projects_loading}</p>
            </div>
          ) : (
                ...
                <div key={p.id} className="group flex flex-col h-full">
                  ...
                  {p.technologies && p.technologies.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-auto mb-6">
                        ...
                        <span key={t} className="text-[9px] font-black px-3 py-1 bg-white/5 rounded-full uppercase tracking-widest text-blue-400">{t}</span>
                        ))}
                    </div>
                  )}
                  <div className="flex gap-4">
                    {p.github_link && <a href={p.github_link} target="_blank" rel="noreferrer" className="text-xs font-black uppercase border-b-2 border-transparent hover:border-blue-600 pb-1 transition-colors">{lang === 'es' ? 'Ver Código' : 'View Code'}</a>}
        ...
        <section className="mb-40 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center bg-white/[0.02] rounded-[3rem] p-12 md:p-20 border border-white/5">
            <div>
              <h2 className="text-sm font-black text-blue-500 uppercase tracking-[0.4em] mb-8">{t.personal_stack_title}</h2>
              <p className="text-3xl font-bold leading-tight mb-8">{t.personal_stack_desc1}</p>
              <p className="text-gray-400 leading-relaxed">{t.personal_stack_desc2}</p>
            </div>
        ...
        <section id="contacto" className="mb-40 bg-blue-600 rounded-[3rem] p-12 md:p-24 text-black overflow-hidden relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 relative z-10">
            <div>
              <h2 className="text-6xl md:text-8xl font-black tracking-tighter leading-[0.8] mb-10 uppercase">{t.contact_title.split(' ')[0]}<br/>{t.contact_title.split(' ')[1]}</h2>
              <p className="text-blue-950 font-bold text-xl max-w-xs">{t.contact_subtitle}</p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-8">
              <input type="text" placeholder={t.form_name} className="w-full bg-transparent border-b-2 border-black/20 p-4 placeholder:text-blue-900 font-black focus:border-black outline-none transition-all uppercase" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} required />
              <input type="email" placeholder={t.form_email} className="w-full bg-transparent border-b-2 border-black/20 p-4 placeholder:text-blue-900 font-black focus:border-black outline-none transition-all uppercase" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} required />
              <textarea placeholder={t.form_msg} rows="4" className="w-full bg-transparent border-b-2 border-black/20 p-4 placeholder:text-blue-900 font-black focus:border-black outline-none transition-all uppercase resize-none" value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})} required></textarea>
              <button className="group bg-black text-white px-12 py-5 rounded-full font-black text-sm uppercase flex items-center gap-4 hover:scale-[1.02] transition-all">
                {t.form_btn} <span className="group-hover:translate-x-2 transition-transform">→</span>
              </button>
